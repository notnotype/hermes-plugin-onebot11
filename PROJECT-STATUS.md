...
- **阶段**：`feat/t7-onebot11-media-runtime-policy` 正在收口媒体投递、纯文本、控制面通知和运行时策略 reload。shared session、schema 11、TurnAnchor、权限、Agent 最终回复图片、插件自有 pi-ai helper 和群级 `/new`、`/reset`、`/clear` 公共命令桥接已在 master 基线成立；本分支尚未合并或部署 Arch。
...
| `adapter.py` | 进行中 | Hermes glue、shared session、入站访问策略、hooks、群级 slash command、工具 handler、群 turn 👀 指示器、Agent 最终回复的文本/图片出站生命周期、base64 segment、同轮媒体去重、纯文本、显式控制面消息、运行时 policy snapshot/reload、媒体回收、统一配置解析、raw self_id、临时摘要注入、operation resolve、pi-ai selector，以及按 event metadata 恢复 worker/async 之间的精确 binding；通用 `send_message`/cron plugin media 不是本轮可靠性合同 |
...
- 在本 worktree 设置 `PYTHONPATH=C:\Users\notnotype\AppData\Local\hermes\hermes-agent` 后，`pytest -q`：`289 passed, 1 skipped`。skip 只表示无 Hermes gateway 的环境会跳过 adapter 集成测试。
...
- `scripts/verify_hermes_integration.py` 在临时 `HERMES_HOME` 下通过：`tools=9 hooks=5 pi_ai_trigger=True reconnect=True slash_commands=True`；本地 Hermes 集成 smoke 和 Node helper 仍是独立验收证据，不能由本地 pytest 自动推断。
- 当前未声称本分支已部署 Arch，也未把合成 WS payload 当成真人 QQ Agent pipeline 验收。

## 外部联调状态

2026-08-10 通过 `ssh arch` 做只读复核，严格固定机器人 QQ `3101482118`、唯一允许群 `1072992996` 和唯一允许私聊用户 `2056963663`：Arch 插件 checkout 为 `master`、commit `fd246b7`、版本 `0.6.0`，`hermes-gateway.service` 为 active，OneBot queue schema 为 `11`。本分支的 Task 7 改动尚未部署；本次复核没有向任何 QQ 发送消息，也没有修改 Arch 配置、SQLite 或 session。
...
- 本轮不纳入 OneBot 12、语音转写、复杂管理后台、RAG/向量库、运行时自优化、queued `⏳` reaction 和强制语义摘要模型。
