...
- `/onebot resolve retry` 为可验证的 `message`/`operator` anchor 创建新的 request id，保留原 authority、固定 batch 边界和 reaction 状态；`legacy` hold 不猜测 authority，不自动 retry。
...
schema v9 把可证明的一对一旧 trigger 迁移为 message anchor。旧 LLM trigger、活跃 lease、uncertain/failed batch 或缺失锚点的记录不能安全推断 authority，转为 `legacy hold` 或未锚定 pending，由管理员处理后重新明确触发。关闭和恢复路径始终重新应用当前群白名单；旧 turn 被 fencing 后不能继续访问 SQLite、工具或 OneBot API。
