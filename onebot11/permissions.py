    ...
    *,
    ...
    return (
        ...
        + (
            ...
            "媒体回传时 manifest 是唯一事实来源：只能使用 manifest.evidence.mediaFiles 的安全绝对路径，"
            "复制到受控媒体根并复核 PNG 魔数、大小和 realpath 后才允许输出 MEDIA:<absolute-path>；"
        ...
        + (
            ...
            "记录失败但仍继续把已验证结论回复给用户。"
            "媒体回传必须 fail-closed：截图或其它文件生成后，必须先读取项目 adapter 的 manifest；"
            ...
            "runner stdout、manifest.evidence.files、仓库路径、evidence 路径、截图文件名或任意裸路径都不是媒体授权来源，"
