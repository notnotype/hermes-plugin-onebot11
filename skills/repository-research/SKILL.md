...
先读取已有 evidence、项目文档和 Skill；已有结论足够回答时直接复用，不新建客服记录，也不把分流理由写进用户回复。
同批次中与当前产品问题无关的闲聊、价格、token 费用或图片话题静默忽略，不逐条回应。

如果结论依赖产品版本、commit、构建号或 provider 实现，而当前消息和已有 evidence 没有精确版本标识，立即暂停调研和委派，先向用户询问版本/构建号。
...
child 没有完成写入和复读证明前，父 agent 不得声称已归档；目录不可写时记录内部阻塞，但仍可回复已经验证的结论。用户未询问归档状态时，最终回复只保留当前问题的结论、必要证据和下一步。
## 阶段二：环境准备
